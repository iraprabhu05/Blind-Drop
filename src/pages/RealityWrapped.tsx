...
const RealityWrapped: React.FC = () => {
  ...
  const sections = [
    ...
    >
      <h1 className="text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-neon-teal to-neon-violet mb-4 animate-shimmer">
        Reality Check Wrapped
      </h1>
      <p className="text-2xl text-muted-foreground">
        Your listening biases, revealed.
      </p>
    </motion.div>,
      ...
      <div className="flex justify-center items-end gap-16">
        <div className="flex flex-col items-center">
          <motion.div
            initial={{ height: 0 }}
            animate={{
              height: `${mockWrappedData.ratingComparison.blindVoted}%`,
            }}
            transition={{ duration: 1 }}
            className="w-24 bg-gradient-to-t from-neon-violet to-indigo-700 rounded-t-lg shadow-neon-violet-strong mb-4"
          />
          <p className="text-2xl font-bold">
            <Counter to={mockWrappedData.ratingComparison.blindVoted} />%
          </p>
        ...
        <div className="flex flex-col items-center">
          <motion.div
            initial={{ height: 0 }}
            animate={{
              height: `${mockWrappedData.ratingComparison.visibleVoted}%`,
            }}
            transition={{ duration: 1 }}
            className="w-24 bg-gradient-to-t from-neon-teal to-cyan-700 rounded-t-lg shadow-neon-teal-strong mb-4"
          />
          <p className="text-2xl font-bold">
            <Counter to={mockWrappedData.ratingComparison.visibleVoted} />%
          </p>
          <p className="text-muted-foreground">Visible</p>
        </div>
      </div>
      <p className="mt-8 text-xl max-w-md mx-auto">
        {mockWrappedData.ratingComparison.insight}
      </p>
    </motion.div>,
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-8 max-w-2xl mx-auto">
        <div className="bg-card/30 backdrop-blur-lg p-6 rounded-lg border border-border/20 shadow-lg">
          <p className="text-5xl font-bold text-neon-teal">
            <Counter to={mockWrappedData.gamifiedStats.blindGuesses} />
          </p>
          <p className="text-muted-foreground mt-2">Blind Guesses</p>
        </div>
        <div className="bg-card/30 backdrop-blur-lg p-6 rounded-lg border border-border/20 shadow-lg">
          <p className="text-5xl font-bold text-neon-teal">
            <Counter to={mockWrappedData.gamifiedStats.correctGuesses} />
          </p>
          <p className="text-muted-foreground mt-2">Correct Guesses</p>
        </div>
        <div className="bg-card/30 backdrop-blur-lg p-6 rounded-lg border border-border/20 shadow-lg">
          <p className="text-5xl font-bold text-neon-violet">
            <Counter to={mockWrappedData.gamifiedStats.reveals} />
          </p>
          <p className="text-muted-foreground mt-2">Reveals</p>
        </div>
        <div className="bg-card/30 backdrop-blur-lg p-6 rounded-lg border border-border/20 shadow-lg">
          <p className="text-5xl font-bold text-neon-violet">
            {mockWrappedData.gamifiedStats.averageReactionTime}
          </p>
    ...
    >
      ...
      <div className="bg-card p-8 rounded-lg shadow-lg text-center">
        <p className="text-7xl font-bold">
          {mockWrappedData.indieDiscovery.newIndieArtists}
        </p>
        <p className="text-xl mt-2">New Indie Artists Discovered</p>
      </div>
      <p className="mt-8 text-xl max-w-md text-center">
        {mockWrappedData.indieDiscovery.insight}
      </p>
    </motion.div>,

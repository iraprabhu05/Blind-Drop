      ...
      {tracks.map((track) => (
        ...
        >
          <div className="relative">
            <img src={track.cover} alt={track.title} className="w-full h-48 object-cover transition-transform duration-300 group-hover:scale-105" />
          ...
          <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
              <button className="p-2 rounded-full bg-black/50 hover:bg-violet-600 backdrop-blur-sm"><Edit size={16}/></button>
              <button className="p-2 rounded-full bg-black/50 hover:bg-red-600 backdrop-blur-sm"><Trash size={16}/></button>
          </div>
        </div>
      ))}
    </div>

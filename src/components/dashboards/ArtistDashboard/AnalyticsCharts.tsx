...
const chartCardStyle = "bg-[#1C1C1C] p-6 rounded-2xl border border-gray-800 shadow-xl";
    ...
    <div className={chartCardStyle}>
        ...
        <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
                <LineChart data={analyticsData.playsOverTime.data.map((val, i) => ({ name: analyticsData.playsOverTime.labels[i], plays: val }))}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
                    <XAxis dataKey="name" stroke="#9CA3AF" />
                    <YAxis stroke="#9CA3AF" />
                    <Tooltip contentStyle={{ backgroundColor: '#1F2937', border: 'none' }} />
                    <Line type="monotone" dataKey="plays" stroke="#A78BFA" strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 8 }} />
                </LineChart>
            </ResponsiveContainer>
        </div>
    </div>
...
const DemographicsChart = () => {
    const data = analyticsData.listenerDemographics.labels.map((label, i) => ({ name: label, value: analyticsData.listenerDemographics.data[i] }));
...
const TrackComparisonChart = () => (
    <div className={`${chartCardStyle} col-span-1 lg:col-span-2`}>
        ...
        <div className="h-96">
            <ResponsiveContainer width="100%" height="100%">
                <BarChart data={analyticsData.trackComparison.labels.map((label, i) => ({ name: label, plays: analyticsData.trackComparison.data[i] }))} layout="vertical">
                    <CartesianGrid strokeDasharray="3 3" stroke="#374151" />

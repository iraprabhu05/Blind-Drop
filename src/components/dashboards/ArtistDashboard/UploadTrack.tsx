...
const UploadTrack = () => {
  ...
  } = useDropzone({ onDrop: onAudioDrop, accept: { "audio/*": [] } });
  const {
    ...
    <div className="space-y-8 max-w-4xl mx-auto animate-fade-in">
      <header>
        ...
        <p className="text-gray-400 mt-2">
          Add your latest creation to Blind Drop.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
        ...
        <div className="space-y-6">
          ...
          <div
            {...getAudioRootProps()}
            className={`p-10 border-2 border-dashed rounded-xl text-center cursor-pointer transition-colors ${isAudioDragActive ? "border-violet-500 bg-violet-500/10" : "border-gray-700 hover:border-violet-600"}`}
          >
            <input {...getAudioInputProps()} />
            <UploadCloud className="mx-auto text-violet-400 mb-2" size={40} />
            <p className="font-semibold">
              {audioFile ? audioFile.name : "Drag & drop audio file or click"}
            </p>
          ...
          <div
            {...getCoverRootProps()}
            className={`p-10 border-2 border-dashed rounded-xl text-center cursor-pointer transition-colors ${isCoverDragActive ? "border-violet-500 bg-violet-500/10" : "border-gray-700 hover:border-violet-600"}`}
          >
            <input {...getCoverInputProps()} />
            <ImageIcon className="mx-auto text-pink-400 mb-2" size={40} />
            <p className="font-semibold">
              {coverFile ? coverFile.name : "Drag & drop cover art or click"}
            </p>
        ...
        <div className="bg-[#1C1C1C] p-8 rounded-2xl border border-gray-800">
          <div className="space-y-6">
            ...
            />
            <Input
              placeholder="Genre (e.g., Cyber-Funk)"
              className="bg-gray-800 border-gray-700 h-12"
            />
            <Input
              placeholder="Mood Tags (e.g., Energetic, Dark)"
              className="bg-gray-800 border-gray-700 h-12"
            />
            <Textarea
      ...
      <div className="space-y-4">
        {uploading && (
          <Progress
            value={progress}
            className="w-full h-3 [&>*]:bg-violet-500"
          />
        )}
        <Button
          ...
          disabled={uploading}
          size="lg"
          className="w-full font-bold text-lg bg-violet-600 hover:bg-violet-700"
        >

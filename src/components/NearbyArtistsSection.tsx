...
export const NearbyArtistsSection = ({ playSong }: NearbyArtistsSectionProps) => {
  ...
  const filteredArtists = useMemo(() => {
    return artistsWithDistance
      .filter(artist => artist.distance !== undefined && artist.distance <= radius)
  ...
  if (error) {
    return <LocationBanner message="Could not access your location. Please enable location services in your browser settings." />;
  }
    ...
    <div>
      ...
      {filteredArtists.length > 0 ? (
        ...
        <div className="text-center py-12">
          <p className="text-muted-foreground">No artists found within {radius}km. Try expanding your radius.</p>
        </div>
      )}
    </div>
  );
